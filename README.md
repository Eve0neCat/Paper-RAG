...
面向学术文献的**长文本、复杂逻辑**场景，基于 **FastAPI + LlamaIndex + 阿里云通义千问** 构建。
...
```
用户提问
  └─(3) 查询改写：结合短期历史 + 长期记忆召回，消解指代 → 学术检索 query
        └─(2) 路由检索：在一/二级标题(路由)节点中定位 Top-K 章节
              └─(4) 深度召回（限定章节范围）
                    ├─ BM25（字词频率）
                    └─ Milvus 向量（bge-m3 语义）
                    └─(5) 融合(RRF / 加权) → bge-reranker-v2-m3 重排 → Top-N
